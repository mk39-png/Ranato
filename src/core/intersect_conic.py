...
from src.core.common import (Matrix3x1r, Matrix3x2f, PlanarPoint1d, Vector2f,
                             Vector3f, logger, todo)
from src.core.conic import Conic
from src.core.convex_polygon import ConvexPolygon
from src.core.polynomial_function import quadratic_real_roots


def intersect_conic_with_line(C_param: Conic, L_coeffs: Vector3f) -> list[float]:
    """
    ...
    :return intersections: list of intersection points in the domain of the
    conic
    ...
    todo("Check if need to implement the rest of the C++ code below...")
...
def intersect_conic_with_convex_polygon(conic: Conic,
    ...
    for i, L_coeffs in enumerate(polygon_boundary_coeffs):
        intersections_poly = intersect_conic_with_line(conic, L_coeffs)
    ...
    logger.info("Intersections: %s", intersections_poly)
    ...
    p_sample = conic(t_sample).flatten()
    logger.info("Sampling at %s", t_sample)

    # FIXME: contains() made to work with 2D planarpoint, change it to work with 1D
    if convex_polygon.contains(p_sample):
...
def intersect_conic_in_cone_patch(conic: Conic,
    ...
    polygon_boundaries_2d: list[Matrix3x1r] = convex_polygon.boundary_segments
    ...
    intersections: list[float] = intersect_conic_with_line(conic, L_coeffs)

    # Check that there are precisely two intersections
    if len(intersections) > 1:
        logger.error("More than two intersections found in cone patch")
        # NOTE: below returning conic and (-1, -1) as placeholders
        return False, conic, (-1, -1)
    ...
    if conic_segment.domain.is_bounded_above():
        ...
        # TODO: below should return placeholders.... or whatnot
        return False, conic_segment, line_intersection_indices
...
def check_if_conic_intersects_cone_patch_domain(conic: Conic,
    ...
    assert domain_point_0.ndim == 1
    assert domain_point_1.ndim == 1
