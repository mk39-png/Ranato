...
from src.contour_network.project_curves import project_curves
...
def test_project_curves_spot_mesh() -> None:
    ...
    spatial_curves: list[RationalFunction] = deserialize_rational_functions(filepath+"spatial_curves.json")
    frame: Matrix3x3f = np.array([[1, 0, 0],
                                  [0, 1, 0],
                                  [0, 0, 1]])

    planar_curves_control: list[RationalFunction] = deserialize_rational_functions(filepath+"planar_curves.json")
